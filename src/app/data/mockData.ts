// ISAS mock data layer — mirrors the SRS UML class model.
// All access is isolated here so real REST calls can be swapped in later.
...
export interface User {
  ...
  role: Role;
  // role specifics
  enrolledSubjects?: string[]; // Student
  subjectsTaught?: string[]; // Teacher
  section?: string;
}
...
export interface Attendance {
  ...
  markedBy: string; // studentID (self) or teacherID
}
...
export interface Assignment {
  ...
  completed: boolean;
  createdBy: string; // studentID or teacherID
...
// ---- Roster of students (for teacher views) ----
...
// ---- Attendance seed (last ~20 sessions per subject for s1) ----
